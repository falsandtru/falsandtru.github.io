...
次の型はすべてまたはほとんどの型が代入可能となり型制約として機能しない場合があるため注意を要する。
...
`undefined`値は`void 0`で随時生成して使用しなければならず`undefined`変数の参照を禁止する。
...
10進数で15桁（正確には2^53/2-1）を超える整数は精度が保証されないため使用禁止とし、文字列化して管理する。
...
型定義を共有する目的ためEnumが公称型であることが不都合であることがある。
...
イディオム化可能な普遍的な処理に対して丸括弧を利用して冗長なブロックを削除することを許可する。
...
スコープからの脱出は必要のない限り上位のスコープに対するものから記述する。
...
ループの中のような非常に短い間隔で繰り返し実行される場所およびそこで呼ばれる関数内でクロージャーを作らない。
関数定義はコストが高い。
...
クロージャーの名前の重複を避ける場合は名前の末尾にアンダースコアを追加または削除して名前を変える。
メソッドにおいても同様とする。
...
コマンドの戻り値は原則として空または実行結果の識別子ないし操作方法のいずれかに限る。
